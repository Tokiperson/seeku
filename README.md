...
[![Version](https://img.shields.io/badge/version-v0.1.0--rc.1-blue)](pubspec.yaml)
...
当前版本：`v0.1.0-rc.1`（pubspec: `0.1.0-rc.1+5`）

这一阶段已经完成：

- 🗓️ 周视图 / 日视图课表展示，支持重庆大学常见节次结构。
...
用 Inno Setup Compiler 打开该脚本并 Compile，或使用命令行：

```powershell
& "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer\seeku.iss
```

安装包默认输出到：

```text
dist\inno\SeekU-v0.1.0-rc.1-setup.exe
```
...
Current version: `v0.1.0-rc.1` (pubspec: `0.1.0-rc.1+5`)

Implemented in this release candidate:

- 🗓️ Week and day schedule views using CQU-style teaching sections.
...
Open it with Inno Setup Compiler and compile, or run:

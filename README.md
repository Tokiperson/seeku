...
[![Version](https://img.shields.io/badge/version-v0.1.1-blue)](pubspec.yaml)
...
```text
installer\seeku.iss
installer\seeku-v0.1.1.iss
```

手动构建后，可用 Inno Setup Compiler 打开对应 `.iss` 脚本并 Compile，或使用命令行：

```powershell
& "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer\seeku-v0.1.1.iss
```

安装包默认输出到：

```text
dist\inno\SeekU-v0.1.1-setup.exe
```
...
Current version: `v0.1.1` (pubspec: `0.1.1+8`)
...
2. Choose `Latest` or a specific version such as `v0.1.1`.
3. Download `SeekU-v0.1.1-setup.exe`.

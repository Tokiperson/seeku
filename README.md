...
[![Version](https://img.shields.io/badge/version-v0.1.1--snapshot-blue)](pubspec.yaml)
...
当前版本：`v0.1.1-snapshot`（pubspec: `0.1.1-snapshot+7`）
...
- 🔗 引入 CQU-Openlib 自动资源匹配功能，课程详情页可展示最佳相关资源链接，并支持展开候选结果。
...
- Openlib 匹配依赖 CQU-Openlib 公开索引与网络状态，当前只展示外部链接和摘要元数据，不下载资源文件。
...
- **Openlib 自动资源匹配**：课程详情页会根据课程名检索 CQU-Openlib，展示最佳相关资源链接，并缓存标题、摘要、链接与匹配分数。
...
2. 选择 `Latest` 或目标版本，例如 `v0.1.1-snapshot`。
3. 下载 `SeekU-v0.1.1-snapshot-setup.exe`。
...
```text
installer\seeku.iss
installer\seeku-v0.1.1-snapshot.iss
```

手动构建后，可用 Inno Setup Compiler 打开对应 `.iss` 脚本并 Compile，或使用命令行：

```powershell
& "C:\Program Files (x86)\Inno Setup 6\ISCC.exe" installer\seeku-v0.1.1-snapshot.iss
```

安装包默认输出到：

```text
dist\inno\SeekU-v0.1.1-snapshot-setup.exe
```
...
lib/
  ...
  features/
    ...
    openlib/           # CQU-Openlib 资源检索、匹配和缓存
...
- [`lib/features/openlib/data/cqu_openlib_resource_provider.dart`](lib/features/openlib/data/cqu_openlib_resource_provider.dart)：CQU-Openlib 搜索索引检索、课程资源匹配与本地缓存。
...
- 继续完善 CQU-Openlib 资源匹配质量、缓存策略与摘要展示。
...
Current version: `v0.1.1-snapshot` (pubspec: `0.1.1-snapshot+7`)
...
- 🔗 Introduced automatic CQU-Openlib resource matching on course detail pages, with best-link display and expandable candidates.
...
- Openlib matching depends on the public CQU-Openlib index and network conditions; SeekU only displays external links and cached metadata, without downloading resource files.
...
- **Automatic Openlib resource matching**: course detail pages query CQU-Openlib by course name, show the best related resource link, and cache title, summary, URL, and match score metadata.
...
2. Choose `Latest` or a specific version such as `v0.1.1-snapshot`.
3. Download `SeekU-v0.1.1-snapshot-setup.exe`.
...
lib/
  ...
  features/
    ...
    openlib/           # CQU-Openlib lookup, matching, and metadata cache
...
- [`lib/features/openlib/data/cqu_openlib_resource_provider.dart`](lib/features/openlib/data/cqu_openlib_resource_provider.dart): CQU-Openlib search-index lookup, course-resource matching, and local cache.
...
- Continue improving CQU-Openlib matching quality, cache behavior, and summary display.

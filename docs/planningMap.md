...
- 教务导入仍规划采用内置 WebView：用户自行登录并打开“我的课表”，SeekU 抓取当前页面内容后解析；当前 snapshot 尚未接入 WebView。
...
- CQU-Openlib 已在 v0.1.1-snapshot 引入第一阶段自动资源匹配：课程详情页展示相关资源链接与候选结果，只缓存标题、链接、摘要、匹配分数和缓存时间，不默认下载或搬运资源文件。
- 账号体系预计 v0.3 以后上线，云同步预计 v0.5 以后上线。

## 当前状态

- 项目已从 v0.1alpha / snapshot 技术切片推进到 `v0.1.1-snapshot`（pubspec: `0.1.1-snapshot+7`）。
...
- 已同步 `README.md`、`ABOUT.md`、`CONTACT.md` 与用户协议到 `v0.1.1-snapshot`，README 已加入 GitHub Releases 下载导览。
...
CQU-Openlib 是重庆大学开源资源共享计划，SeekU 已在 v0.1.1-snapshot 接入第一阶段自动资源匹配。当前只缓存资源标题、链接、摘要、匹配分数与缓存时间，不默认下载文件。
...
### v0.1.1-snapshot：Openlib 自动资源匹配

- 版本号更新为 `v0.1.1-snapshot` / `0.1.1-snapshot+7`。

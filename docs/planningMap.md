...
- v0.1 优先 Windows 桌面端；Android 在 v0.2 以后发布。
- 教务入口为 `https://my.cqu.edu.cn/workspace/home`。
- 教务导入采用内置 WebView：用户自行登录并打开“我的课表”，SeekU 抓取当前页面内容后解析。
- v0.1 即支持多学期，课程数据必须绑定到学期。
- 离线能力优先满足查看已保存课表、课程详情、已缓存资源摘要与手动编辑。
- 学校统一节次时间表可先在代码中预设，具体时间后续开发时补充。
- 早期 API Key 可接受本地明文配置，后续再升级为安全存储。
- 大模型接入优先考虑 DeepSeek 与 Moonshot。
- CQU-Openlib 在 v0.2 阶段先做“链接 + 摘要”，不默认下载或搬运资源文件。
- 账号体系预计 v0.3 以后上线，云同步预计 v0.5 以后上线。

## 当前状态

- 项目已完成 v0.1alpha 技术切片实现。
- 已建立 `app/`、`core/`、`features/`、`shared/` 模块化工程结构。
- 已接入 `go_router`、`flutter_riverpod`、`drift`、`shared_preferences`、`file_picker`、`path_provider` 等 v0.1alpha 依赖。
- 已实现 Windows 桌面端本地 SQLite 课表存储、默认学期、默认 CQU 节次表、课程 CRUD、导入批次与设置项。
- 已实现 CQU 蓝白风格的周视图、日视图、课程详情、新增 / 编辑 / 删除课程、导入占位页和设置页。
- 已更新 `README.md` 为 SeekU 项目说明。
- 已完成规则、数据库和 Widget smoke 测试，`dart analyze` 与 `flutter test` 通过。
...
- 支持导入原始文件快照开关。
- Excel 导入 alpha 行为：选择 `.xlsx`、`.xls`、`.csv` 文件，创建 `ImportBatch`，按设置决定是否保存原始文件副本，并展示“等待样例格式适配”的导入预览占位状态。
- 已固定导入管线接口：`ImportSource`、`RawImportSnapshot`、`CourseDraft`、`ImportValidator`、`ConflictDetector`。
- 已支持周次表达式与节次表达式解析测试。
...
- PDF / 截图导入：v0.2 通过 OCR 或大模型视觉能力识别，再要求模型返回固定 JSON。
...
v0.2 接入 DeepSeek 与 Moonshot，所有模型能力必须通过统一接口调用，不直接耦合 UI。

规划接口：

```dart
abstract class LlmClient {
  Future<StructuredSchedule> parseScheduleText(String text);
  Future<List<LearningAdvice>> generateCourseAdvice(Course course);
}
```
...
v0.1alpha 暂未接入 `excel`、`webview_windows`、`dio`，待 Excel 样例和教务 DOM / HTML 样例确认后再纳入。
...
- `features/ai/`：DeepSeek / Moonshot 统一接口、Mock 服务、请求日志。
...
- 在 v0.1alpha 基础上补齐完整多学期管理。
- 基于脱敏样例实现 Excel 真实解析与字段映射。
- 完善导入预览页，支持课程草稿编辑、校验结果展示和冲突确认。
...
- Excel 导入需要脱敏样例文件，确认学校导出格式。
- 教务页面折叠课程必须处理，否则可能漏课。
- 教务网站结构可能变动，需要保存快照与错误反馈机制。
- OCR 和大模型识别存在误差，必须保留导入预览和人工修正。
...
1. 获取脱敏 Excel 样例文件，确认 CQU 常见课表导出格式。
2. 基于样例实现 Excel 解析、字段映射和课程草稿生成。
3. 完善导入预览页，支持草稿编辑、冲突展示、确认入库。
4. 获取脱敏教务课表 HTML / DOM 片段，确认折叠课程和整周课程结构。
5. 接入 `webview_windows`，实现教务登录后页面抓取与快照保存。
6. 补齐完整多学期创建 / 编辑 / 切换体验。
7. 继续优化 Windows 桌面端课表密度、可读性和小窗口响应式布局。

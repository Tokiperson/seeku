...
- 项目已完成 v0.1alpha 技术切片，并推进到 `0.1.0-snapshot.2+2`。
...
- 已实现 CQU 蓝白风格的周视图、日视图、课程详情、新增 / 编辑 / 删除课程、多源导入页和设置页。
- 已实现 Excel / CSV 真实解析、导入学期选择、导入预览、冲突检测和确认入库闭环。
- 已初步接入 AI 核心框架：Moonshot / Kimi API 管理、PDF 文本抽取、图片视觉识别、AI 结构化课表解析、AI 核心状态检测与 API Key 配置。
- 已实现导入教室到校区的自动推断：教室首字符为英文时写入大写校区，否则留空。
- 已更新 `README.md` 为 SeekU 项目说明。
- 已完成规则、数据库、导入解析、AI JSON 解析、AI 设置和 Widget smoke 测试，`dart analyze` 与 `flutter test` 通过。
...
- 未配置时每次启动提醒配置，并允许从首次打开起 5 天内使用内置 API Key 试用。

...
- 教务导入仍规划采用内置 WebView：用户自行登录并打开“我的课表”，SeekU 抓取当前页面内容后解析；当前版本尚未接入 WebView。
...
- CQU-Openlib 已在 v0.1.1 引入第一阶段自动资源匹配：课程详情页展示相关资源链接与候选结果，只缓存标题、链接、摘要、匹配分数和缓存时间，不默认下载或搬运资源文件。
- 账号体系预计 v0.3 以后上线，云同步预计 v0.5 以后上线。

## 当前状态

- 项目已从 v0.1alpha / snapshot 技术切片推进到 `v0.1.1`（pubspec: `0.1.1+8`）。
- 已建立 `app/`、`core/`、`features/`、`shared/` 模块化工程结构。
- 已接入 `go_router`、`flutter_riverpod`、`drift`、`shared_preferences`、`file_picker`、`path_provider`、`excel`、`pointycastle`、`url_launcher` 等依赖。
...
- 已引入 CQU-Openlib 自动资源匹配功能：课程详情页根据课程名检索公开索引，展示最佳相关资源链接和候选结果，并通过本地 SQLite 缓存资源元数据。
- 已同步 `README.md`、`ABOUT.md`、`CONTACT.md` 与用户协议到 `v0.1.1`，README 已加入 GitHub Releases 下载导览。
...
5. 持续扩展 DeepSeek / Moonshot 与 CQU-Openlib 能力，形成学习资源推荐和学习建议能力。
...
- 手动新增、编辑、删除课程。
- 教务 WebView 导入（仍在规划，snapshot 尚未接入）。
- Excel 导入。
- 导入预览与冲突提示。
- 重庆大学统一节次表预设接口。
- 本地数据库持久化。
...
CQU-Openlib 是重庆大学开源资源共享计划，SeekU 已在 v0.1.1 接入第一阶段自动资源匹配。当前只缓存资源标题、链接、摘要、匹配分数与缓存时间，不默认下载文件。
...
- `features/openlib/`：CQU-Openlib 搜索索引检索、课程资源匹配、摘要元数据缓存与推荐展示预留。
...
### v0.1.1：Openlib 自动资源匹配与 Windows 启动体验

- 版本号更新为 `v0.1.1` / `0.1.1+8`。
- 新增 `features/openlib/` 数据层与领域缓存模型，接入 CQU-Openlib 公开搜索索引。
- 课程详情页新增“相关资源链接”属性，自动展示最佳匹配资源，并支持展开候选结果。
- 新增 `openlib_resource_cache` 本地缓存表，缓存标题、链接、摘要、匹配分数与缓存时间，默认 7 天有效。
- 通过 `url_launcher` 在 Windows 端唤起系统默认浏览器打开 Openlib 外部链接。
- 修复 Openlib 搜索索引 `course/...` 路径格式导致课程页被过滤的问题，并将公开索引请求超时调整为 30 秒。
- 修复课程详情页 rebuild 导致 Openlib 检索反复进入 loading 的问题。
- 补充 Openlib provider、缓存、离线回退和课程详情展示测试，并通过 `flutter analyze` 与 `flutter test`。
- Windows 端新增原生启动动画，安装包脚本同步到 `installer/seeku.iss` 与 `installer/seeku-v0.1.1.iss`。
...
- AI 结构化解析课表。
- 持续完善 CQU-Openlib 课程资源匹配、摘要展示、相关性说明与用户手动选择能力。
- API Key 配置与请求日志。
...
- CQU-Openlib 当前基于公开搜索索引检索，仍需持续确认索引结构变化、缓存策略和请求失败体验。
